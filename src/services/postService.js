const Post = require("../models/Post");
class PostService {
  async createPost(info) {
    const createPost = Post.create(info);

    const alredyExistingPost = await Post.findOne({
      where: { id: info.id, user_id: info.user_id }
    });
    if (alredyExistingPost) {
      throw new Error("Esse post já existe!");
    } else {
      return await createPost;
    }
  }

  async listPosts(id, user_id) {
    if (!id) {
      throw new Error("o id para do post não existe");
    } else if (!user_id) {
      throw new Error("O usuário não existe.");
    } else {
      return await Post.findAll({ where: { deleted_at: null } });
    }
  }

  async showPost(id) {
    try {
      const showPost = await Post.findOne(id);
      return showPost;
    } catch (error) {
      console.log("🚀 ~ PostService ~ showPost ~ showPost:", showPost);
      throw new Error(error);
    }
  }

  async updatePost(id, info) {
    const post = Post.findByPk(id);
    if (!post) {
      throw new Error("O post em questão não existe.");
    } else {
      const updatedPost = Post.update(info);
      return updatedPost;
    }
  }

  async deletePost(id, user_id) {
    const post = await Post.findByPk(id);
    if (!post) {
      throw new Error("o post não existe para que o mesmo possa ser deletado.");
    }
    if (post.user_id !== user_id) {
      throw new Error("Você não pode para deletar este post.");
    }
    await post.destroy();
    return { message: "O post foi deletado com sucesso!" };
  }
}

module.exports = new PostService();
